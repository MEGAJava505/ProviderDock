...
function createApplication(fetchImpl: typeof fetch = vi.fn<typeof fetch>()): ProviderDockApplication {
  const adapters = new ProviderAdapterRegistry().register(
    new GenericOpenAiAdapter({ secretStore: new MemorySecretStore(), fetchImpl }),
  );
...
async function runCli(application: ProviderDockApplication, argv: string[]) {
  const stdout: string[] = [];
  const stderr: string[] = [];
  const io: CliIo = { stdout: (message) => stdout.push(message), stderr: (message) => stderr.push(message) };
  const code = await runProviderDockCli(argv, { application, io });
